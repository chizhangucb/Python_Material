...
class VendingMachine(object):
    ...
    def vend(self):
        if self.stock == 0:
            ...
            output = "Here is your candy and ${0} change.".format(self.balance - self.price)
            self.balance = 0
            self.stock -= 1
            return output
...
class MissManners(object):
    ...
    def ask(self, command, *args):
        nice_manner = 'please'
        if command.split()[0] != nice_manner:
            return 'You must learn to say please first.'
        key_command = command.split()[1:]
        if not hasattr(self.obj, ' '.join(key_command)):
            return 'Thanks for asking, but I know not how to {0}'.format(' '.join(key_command))
        return getattr(self.obj, ' '.join(key_command))(*args)
