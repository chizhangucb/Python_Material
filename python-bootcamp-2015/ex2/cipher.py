...
# if you did from __future__ import (absolute_import, division, print_function, unicode_literals) before,
# use str(msg).translate(translator)
'svool'
    ...
    def __init__(self, mapping):
        ...
        try:
            if mapping == 'reverse':
                ...
                key = "nmlkjihgfedcbazyxwvutsrqpo"
    ...
    def isvalid(self, message):
        ...
        if not message.isalpha():
            ...
            for i in range(len(list(message))):
                if list(message)[i] not in self.alphabet:
                    return False
            return True
